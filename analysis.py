...
print(f"\nTop 10 by usage:")
print(analysis_df.nlargest(10, 'usage_count')[['geo_id', 'geo_name', 'usage_count', 'gdp_per_working_age_capita', 'usage_per_capita_index']])
...
try:
    ...
    print("Falling back to empirical Bayes approximation...")
...
if not PARTIAL_POOLING_SUCCESS:
    print("\n" + "=" * 60)
    print("EMPIRICAL BAYES SHRINKAGE ESTIMATOR")
    print("=" * 60)
    ...
    print("Shrinkage factors (higher = more weight to data, less shrinkage):")
    print(analysis_df[['geo_id', 'usage_count', 'shrinkage_factor', 'log_usage', 'eb_estimate']].nlargest(10, 'shrinkage_factor').to_string())
...
print("Slopes by income tercile (exploratory):")
for group in ['Low', 'Mid', 'High']:
    ...
    if len(subset) > 3:
        ...
        print(f"  {group} income: β = {model_sub.params['log_gdp']:.3f} (n={len(subset)})")
